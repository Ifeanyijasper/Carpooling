...
def view_user_id(request,pk):
    ...
    if request.is_ajax():
        return render(template_name='broadcast/broadcast_template.html',context={'user':user,'broadcasts':broadcasts},
...
def view_user_name(request,user_id):
    ...
    print(user)
    if request.is_ajax():
        return render(request,template_name='broadcast/broadcast_template.html',context={'user':user,'broadcasts':broadcasts})
    return render(request,'app/user/user_profile.html',{'user':user,'broadcasts':broadcasts})
...
def vehicle_share(request, user_id, vehicle_id):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            form.fields['cost'] = fm.IntegerField(widget=forms.TextInput(attrs={'type':'number', 'min':"10", 'required':True,'placeholder':'In Franc (cfa) ','class':'form-control'}))
...
def view_single_ride(request,vehicle_share_id):    
    ...
    return render(request, 'app/vehicles/view_single_ride.html', context)

@login_required
def view_single_vehicle(request, ride_id):
    if  not request.user.is_authenticated:
        raise Http404
    ride = get_object_or_404(Vehicle, id=ride_id)
...
def requests_driver_view(request,user_id):
    ...
    pass_requests = Request.objects.filter(ride=shared_veh[0]).order_by('reg_date')
...
def timeline(request):
    # ride = get_object_or_404(VehicleSharing)
    # user = get_object_or_404(CustomUser,pk=request.user.id)
    # ride = VehicleSharing.objects.get()
    user = get_object_or_404(CustomUser,pk=request.user.id)
    requests = Request.objects.filter(ride__user=user).order_by('pk')
    rides = VehicleSharing.objects.order_by('-date')
    print(rides)
    context = {
        'rides': rides,
        "request" :requests
    }
    return render(request, 'app/dashboard/dashboard_timeline.html', context)
    # return render(request,'app/dashboard/dashboard_timeline.html',{'notifications':request.user.notifications.unread})
